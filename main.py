...
BOUNCE_SOUND = pygame.mixer.Sound(os.path.join('sfx', 'bounce.wav'))
...
fullscreen_img = pygame.transform.scale(pygame.image.load(os.path.join('pictures', 'fullscreen.png')),
...
class particle():
    ...
    def __init__(self, x, y, x_velocity, y_velocity, width, height, color, gravity_scale, glow, glow_radius):
        self.x = x
        self.y = y
    ...
    def draw(self, display):
        ...
        if self.glow:
            ...
            if self.r == colors[0]:
                self.r = change
            elif self.g == colors[0]:
                self.g = change
            elif self.b == colors[0]:
                self.b = change
            if self.r == colors[2] and not self.r == colors[0]:
                self.r += change
            elif self.g == colors[2] and not self.g == colors[0]:
                self.g = change
            elif self.b == colors[2] and not self.b == colors[0]:
                self.b += change
            if self.r > 255:
                self.r = 255
            if self.g > 255:
            ...
            if self.b < 0:
                self.b = 0
            rect = rect_surf((self.r, self.g, self.b, 0), self.width + self.glow_radius, self.height + self.glow_radius)
            display.blit(rect, (self.x-self.glow_radius/2, self.y-self.glow_radius/2), special_flags=pygame.BLEND_RGBA_ADD)
...
def draw_window(player, mini_square):
    ...
    if pcolor == DARK_RED:
        for t in trail:
            pygame.draw.rect(screen, TRAIL_RED, (
                t[0] + size_remove / 2, t[1] + size_remove / 2, PLAYER_WIDTH - size_remove,
    ...
    for t in mini_trail:
        pygame.draw.rect(screen, ORANGE, (
            t[0] + trail_remove / 2, t[1] + trail_remove / 2, MINI_SQUARE_WIDTH - trail_remove,
    ...
    pygame.draw.rect(shake_screen, RED,
                     (SCREEN_WIDTH / 2 - HEALTH_BAR_WIDTH / 2, 50, HEALTH_BAR_WIDTH, HEALTH_BAR_HEIGHT))
    pygame.draw.rect(shake_screen, GREEN,
                     (SCREEN_WIDTH / 2 - HEALTH_BAR_WIDTH / 2, 50, health * HEALTH_WIDTH_MULTIPLIER, HEALTH_BAR_HEIGHT))
        ...
        if player.y - PLAYER_VEL > 0:
            ...
            if not trail[1][1] == player.y:
                ...
                if dash < DASH_GOAL:
                    ...
                    dash_ccd = 0
                dash_pause = True
                float_texts.append(float_text(player.x, player.y, 10, random.randint(-4, 4), RED, RETRO_TEXT,
                                              ("-" + str(remove_amount))))
                for i in range(15):
                    particles.append(
                        particle(player.x, player.y, random.randrange(-5, 5), random.randrange(-2, 0), 10, 10, DARK_RED,
                                 1, True, 20))
                if health > 0:
...
def mini_square_movement(player, mini_square):
    ...
    if mini_square.colliderect(player) and dashing == False:
        ...
        if dash < 0:
            dash = 0
        float_texts.append(
            float_text(player.x, player.y, random.randint(-10, 10), random.randint(-5, 5), RED, RETRO_TEXT,
                       ("-" + str(remove_amount))))
        HIT_SOUND.play()
        if health > 0:
        ...
        for i in range(30):
            particles.append(
                particle(player.x, player.y, random.randrange(-5, 5), random.randrange(-2, 0), 16, 16, DARK_RED, 1, True, 10))
        for i in range(40):
            particles.append(
...
async def lose_screen():
    ...
    while dead:
        ...
        exit_rect = pygame.Rect(SCREEN_WIDTH - EXIT_IMG_SIZE * 2 + 100, EXIT_IMG_SIZE - 90, EXIT_IMG_SIZE,
                                EXIT_IMG_SIZE)
        fullscreen_rect = pygame.Rect(SCREEN_WIDTH - FULLSCREEN_IMG_SIZE * 2 - 50, FULLSCREEN_IMG_SIZE - 20,
                                      FULLSCREEN_IMG_SIZE, FULLSCREEN_IMG_SIZE)
...
def passive_healing_handler(player, heal_amount, cooldown, active):
    ...
    if heal_cooldown >= cooldown and active and dash_pause == False:
        ...
        for i in range(10):
            particles.append(
                particle(player.x, player.y, random.randrange(-5, 5), random.randrange(-1, 0), 10, 10, GREEN, 1, True, 10))
        float_texts.append(
...
def bouncer_movement(player):
    ...
    for bouncer in bouncers:
        ...
        if bouncer.x + bouncer.width + (bouncers_vel[index][0] * 2) + 5 >= SCREEN_WIDTH and not last_bounces[
                                                                                                    index] == 'right':
            ...
            for i in range(3):
                particles.append(
                    particle(bouncer.x, bouncer.y, random.randrange(-5, 5), random.randrange(-2, 0), 5, 5, CYAN, 1, True, 10))
            ...
            for i in range(3):
                particles.append(
                    particle(bouncer.x, bouncer.y, random.randrange(-5, 5), random.randrange(-2, 0), 5, 5, CYAN, 1, True, 10))
            bounces_survived += SCORE_AMOUNT
        if bouncer.y + bouncer.height + (bouncers_vel[index][1] * 2) + 5 >= SCREEN_HEIGHT and not last_bounces[
                                                                                                      index] == 'down' and corner == False:
            bouncers_vel[index][1] *= -1
...
def health_handler(player):
    ...
    for pack in health_packs:
        if pack.colliderect(player):
            ...
            health += add_amount
            float_texts.append(
                float_text(player.x, player.y, 10, random.randint(-4, 4), GREEN, RETRO_TEXT, ("+" + str(add_amount))))
            ...
            for i in range(30):
                particles.append(
                    particle(player.x, player.y, random.randrange(-5, 5), random.randrange(-1, 0), 10, 10, GREEN, 0, True, 10))
            if health > MAX_HEALTH:
                health = 100
...
def pu_speed_handler(player):
    ...
    for pack in speed_packs:
        if pack.colliderect(player):
            ...
            float_texts.append(float_text(player.x, player.y, -5, random.randint(-2, 2), BLUE, RETRO_TEXT,
                                          ("+" + str(speed_add) + " Speed!")))
            PU_SPEED_SOUND.play()
            for i in range(50):
                particles.append(
                    particle(player.x, player.y, random.randrange(-5, 5), random.randrange(-1, 0), 10, 10, BLUE, 0, True, 10))
        index += 1
...
def pu_live_handler(player):
    ...
    for pack in pu_lives:
        if pack.colliderect(player):
            ...
            for i in range(50):
                particles.append(
                    particle(player.x, player.y, random.randrange(-5, 5), random.randrange(-1, 0), 10, 10, PINK, 0, True, 10))
        index += 1
...
def dash_handler(player, keys_pressed):
    ...
    if dash_pause:
        dash_ccd += 1
        particles.append(
            particle(player.x, player.y, random.randrange(-5, 5), random.randrange(-1, 0), 10, 10, DARK_RED, 1, True, 10))
        if dash_ccd >= bleed:
    ...
    if dashing == True:
        ...
        for b in bouncers:
            ...
            for bt in bouncer_trails[index]:
                if deadb:
                    break
                temprect = pygame.Rect((bt[0] + size_remove / 2, bt[1] + size_remove / 2, BOUNCER_WIDTH - size_remove,
                                        BOUNCER_HEIGHT - size_remove))
                if temprect.colliderect(player):
                    for i in bouncer_trails[index]:
                        for e in range(1):
                            particles.append(
                                particle(i[0], i[1], random.randrange(-5, 5), random.randrange(-1, 0), 10, 10, CYAN, 0, True, 10))
                        deadb = True
                    size_remove += SIZE_REMOVE_AMOUNT
                    ...
                    if deadb == True:
                        ...
                        for i in range(BOUNCER_ADD):
                            ...
                            for i in range(20):
                                particles.append(
                                    particle(randx, randy, random.randrange(-5, 5), random.randrange(-2, 0), 10, 10,
                                             CYAN,
                                             0, True, 10))
                        index = 0
        ...
        for bouncer in bouncers:
            ...
            if player.colliderect(bouncer):
                for i in bouncer_trails[index]:
                    for e in range(5):
                        particles.append(
                            particle(i[0], i[1], 0, -2, 10, 10, CYAN, 0, True, 10))
                    deadb = True
                if deadb == True:
                    bouncers.pop(index)
                    dash_pause = False
                    ...
                    for i in range(BOUNCER_ADD):
                        ...
                        for i in range(30):
                            particles.append(
                                particle(randx, randy, random.randrange(-5, 5), random.randrange(-2, 0), 10, 10, CYAN,
                                         0, True, 10))
                    float_texts.append(float_text(player.x, player.y, -5, random.randint(-2, 2), GREEN, RETRO_TEXT,
                                                  "+1 Score Per Bounce!"))
                    if len(bouncers) > MAX_BOUNCERS:
        ...
        if len(bouncers) < 1:
            ...
            for i in range(40):
                particles.append(
                    particle(randx, randy, random.randrange(-5, 5), random.randrange(-2, 0), 10, 10, CYAN, 0, True, 10))
            for i in range(BOUNCER_ADD):
                bouncers.append(
                    pygame.Rect(randx - BOUNCER_WIDTH / 2, randy - BOUNCER_WIDTH / 2, BOUNCER_WIDTH, BOUNCER_HEIGHT))
                bouncers_vel.append([BOUNCER_VEL_X, BOUNCER_VEL_Y])
        ...
        if dashingamount >= DASH_AMOUNT or player.x + PLAYER_VEL + 20 >= SCREEN_WIDTH or player.x + PLAYER_VEL - 20 <= 0 or player.y + PLAYER_VEL + 20 >= SCREEN_HEIGHT or player.y + PLAYER_VEL - 20 <= 0:
            ...
            for i in range(20):
                particles.append(
                    particle(player.x, player.y, random.randrange(-5, 5), random.randrange(-1, 0), 10, 10, WHITE, 0, True, 10))
            dash = 0
    else:
        pcolor = DARK_RED
    ...
    if keys_pressed[pygame.K_LSHIFT] or keys_pressed[pygame.K_RSHIFT] and dashing == False:
        if dash == DASH_GOAL:
            for i in range(20):
                particles.append(
                    particle(player.x, player.y, random.randrange(-5, 5), random.randrange(-1, 0), 10, 10, WHITE, 1, True, 10))
            dashing = True
            dashingamount = 0
            curdirx = pdirx
            curdiry = pdiry
    ...
    for star in stars:
        if star.colliderect(player):
            ...
            if not len(bouncers) <= MAX_BOUNCERS / (2 / 3):
                bounce_limit += random.randint((bounce_limit - bounces_survived) * -1,
            ...
            for i in range(30):
                particles.append(
                    particle(player.x, player.y, random.randrange(-5, 5), random.randrange(-2, 0), 10, 10, YELLOW, 0, True, 10))
            float_texts.append(float_text(player.x, player.y, 10, random.randint(-4, 4), GREEN, RETRO_TEXT,
                                          ("+" + str(add_amount))))
            for i in range(bounce_purge):
                for i in range(30):
                    particles.append(
                        particle(bouncers[purge_num].x, bouncers[purge_num].y, random.randrange(-5, 5),
                                 random.randrange(-2, 0), 10, 10, CYAN,
                                 0, True, 10))
...
async def main():
    ...
    exit_rect = pygame.Rect(SCREEN_WIDTH - EXIT_IMG_SIZE * 2 + 100, EXIT_IMG_SIZE - 90, EXIT_IMG_SIZE, EXIT_IMG_SIZE)
    fullscreen_rect = pygame.Rect(SCREEN_WIDTH - FULLSCREEN_IMG_SIZE * 2 - 50, FULLSCREEN_IMG_SIZE - 20,
                                  FULLSCREEN_IMG_SIZE, FULLSCREEN_IMG_SIZE)
    ...
    while run:
        ...
        if health <= 0:
            if lives < 2:
                ...
                player = pygame.Rect(SCREEN_WIDTH / 2 - PLAYER_WIDTH / 2, SCREEN_HEIGHT / 2 - PLAYER_HEIGHT / 2,
                                     PLAYER_WIDTH,
                                     PLAYER_HEIGHT)
                DEATH_SOUND.play()
        ...
        if bounces_survived >= bounce_limit:
            if not len(bouncers) == MAX_BOUNCERS:
                ...
                for i in range(40):
                    particles.append(
                        particle(randx, randy, random.randrange(-5, 5), random.randrange(-2, 0), 10, 10, BLUE, 0, True, 10))
            else:
